            ...
            <span class="details">ID Tercero:</span>
                <select name="ID_TERCERO" id="ID_TERCERO" required>
                    <option value="">Seleccione un Tercero</option>
                        ...
                        $conn = new mysqli("localhost", "root", "", "DB_CAJA_FINTECH");
                        $sql = "SELECT ID_TERCERO, NOM_TERCERO FROM CAT_TERCEROS"; // Cambia la tabla y campos según tu base de datos
                        $result = $conn->query($sql);
                        while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row['ID_TERCERO'] . "'>" . htmlspecialchars($row['NOM_TERCERO']) . "</option>";
                        }
                        $conn->close();
                        ?>
                </select>
